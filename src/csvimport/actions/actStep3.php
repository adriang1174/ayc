...
if( !empty($options['file_name']) && !empty($options['mapping']) ) //1st and 2nd step were OK 
{
  ...
  {
    $error = 'Cantidad de filas importadas: 0.';
  }
  //
  // Check proveedor profile. If so, then update IdProveedor, because IdProveedor was not filled in import process
  //
  if( $_SESSION['perfil'] == 'PROVEEDOR')
  {
		$sql = "update empleadosAR set IdProveedor = ".$_SESSION['proveedor']." where idProveedor = 999999";
		//var_dump($sql);
		mysql_query($sql);
  }
}
