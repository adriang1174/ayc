...
$arr_tables = array ('csv' => 'csv','empleadosAr' => 'empleadosAr');
...
if( !empty($_POST["Go"]) ) //form was submitted
{
  ...
  if( is_uploaded_file($_FILES['file_source']['tmp_name']) && UPLOAD_ERR_OK == $errorCode )  //file was uploaded successfully
  {
    ...
    {
      $error = 'No se puede copiar [' . $temp_file .'] a [' . $our_file . ']';
    }
    else
    {
  ...
  {
    $error = coalesce( $uploadErrors[$errorCode], 'Error general al intentar cargar el archivo' );
